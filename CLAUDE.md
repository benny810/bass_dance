...
- **`pca_motion.py`** — The core motion algorithm. Reconstructs joint angles as `mean_pose + Σ(activation_i × component_i)`. The default pipeline runs three layers per frame; a fourth (step) layer is opt-in via `enable_steps=True`:
  ...
  Public knobs: `scale` (global), `pc_weights` (per-PC, length-`n_comp`, pad/truncate to 1.0), `enable_steps` (bool). `pc_weights[i]` multiplies into `amps[i]` so it scales *both* the carrier and the event accent for PC*i* uniformly.
- **`trajectory_generator.py`** — Orchestrates the pipeline. Adds arm joints at `NEUTRAL_STANCE` constant values. Clamps all joints to `JOINT_LIMITS` (from URDF). Applies gaussian smoothing (σ=2 samples). Forwards `scale`, `pc_weights`, `enable_steps` to `generate_pca_motion`. Exports `JOINT_NAMES` (27 joints) and `LOWER_BODY_JOINTS` (13 joints).
